...
from sudoku_board import obtener_candidatos_celda, validar_colocacion_celda, verificar_solucion_valida
...
def busqueda_dfs(board: list[list[str]], size: int, metrics: SolverMetrics, seleccionar_celda: Callable[[list[list[str]], int], tuple[int, int] | None], probar_valores: Callable[[list[list[str]], int, int, int, SolverMetrics], bool]) -> bool:
...
def _expandir_backtracking(board: list[list[str]], size: int, fila: int, columna: int, metrics: SolverMetrics) -> bool:
...
def _expandir_mrv(board: list[list[str]], size: int, fila: int, columna: int, metrics: SolverMetrics) -> bool:
...
def resolver_backtracking(board: list[list[str]], size: int, metrics: SolverMetrics) -> bool:
    return busqueda_dfs(board, size, metrics, _seleccionar_primera_vacia, _expandir_backtracking)

def resolver_backtracking_mrv(board: list[list[str]], size: int, metrics: SolverMetrics) -> bool:
    return busqueda_dfs(board, size, metrics, _seleccionar_mrv, _expandir_mrv)

def resolver_fuerza_bruta(board: list[list[str]], size: int, metrics: SolverMetrics) -> bool:
...
def preprocesar_divide_y_venceras(board: list[list[str]], size: int=9) -> dict[str, object]:
    ...
    for sr in range(0, size, br):
        for sc in range(0, size, bc):
            ...
            subcuadrantes.append({'block_index': indice_bloque, 'origin': (sr, sc), 'empty_cells': celdas_vacias, 'domains': dominios, 'empty_count': len(celdas_vacias)})
            indice_bloque += 1
    return {'size': size, 'block_shape': (br, bc), 'blocks': subcuadrantes, 'total_empty_cells': sum((b['empty_count'] for b in subcuadrantes))}
...
def obtener_resolver(algoritmo: str) -> Callable[[list[list[str]], int, SolverMetrics], bool]:
    ...
    return resolver
ALGORITMOS.update({'backtracking': {'name': 'Backtracking con poda por restricciones', 'fn': resolver_backtracking}, 'brute_force': {'name': 'Fuerza Bruta (sin poda anticipada)', 'fn': resolver_fuerza_bruta}, 'mrv': {'name': 'Backtracking con heurística MRV', 'fn': resolver_backtracking_mrv}})
