"""
Medición, comparación y exportación de métricas experimentales.

Requisitos cubiertos:
 15. Comparar el rendimiento entre algoritmos.
 16. Medir el tiempo de ejecución.
 17. Contar nodos explorados.
 18. Contar backtracks realizados.
 19. Medir consumo de memoria.
 20. Calcular porcentaje de tableros resueltos correctamente.
 21. Exportar métricas obtenidas.
"""
...
def medir_tiempo_ejecucion(funcion: Callable[[], Any]) -> tuple[Any, float]:
    """Ejecuta una función y devuelve (resultado, tiempo_ms)."""
...
def medir_consumo_memoria(funcion: Callable[[], Any]) -> tuple[Any, float]:
    """
    Ejecuta una función midiendo el pico de memoria con tracemalloc.
    Devuelve (resultado, memoria_kb).
    """
    tracemalloc.start()
    try:
        resultado = funcion()
...
def resolver_con_metricas(
    puzzle: list[list[Any]],
    size: int,
    algoritmo: str,
) -> dict[str, Any]:
    """
    Resuelve un puzzle con el algoritmo indicado y recopila métricas completas.
    """
    ...
    tracemalloc.start()
    inicio = time.perf_counter()
    exito = resolver(tablero, size, metrics)  # type: ignore[operator]
    tiempo_ms = (time.perf_counter() - inicio) * 1000
    _, pico = tracemalloc.get_traced_memory()
    tracemalloc.stop()
    ...
    return {
        "success": solucion_valida,
        "solution": tablero,
        "metrics": {
            "execution_time_ms": round(tiempo_ms, 3),
            "nodes_explored": metrics.nodes,
            "backtracks": metrics.backtracks,
            "combinations_tried": metrics.combinations_tried,
            "memory_peak_kb": round(pico / 1024, 3),
            "algorithm": info["name"],
            "algorithm_key": algoritmo,
        },
    }


def comparar_rendimiento_algoritmos(
    puzzle: list[list[Any]],
    size: int,
    algoritmos: list[str] | None = None,
) -> dict[str, Any]:
    """
    Ejecuta varios algoritmos sobre el mismo tablero
    y devuelve una tabla comparativa de métricas.
    """
    ...
    for clave in claves:
        ...
        if clave == "brute_force" and size > 9:
            resultados.append(
                {
                    "algorithm_key": clave,
                    "algorithm": ALGORITMOS[clave]["name"],
                    "success": False,
                    "skipped": True,
                    "reason": "Fuerza Bruta solo disponible para 9×9.",
                }
            )
            continue
        ...
        resultados.append(
            {
                "algorithm_key": clave,
                "algorithm": metricas["algorithm"],
                "success": resultado["success"],
                "execution_time_ms": metricas["execution_time_ms"],
                "nodes_explored": metricas["nodes_explored"],
                "backtracks": metricas["backtracks"],
                "combinations_tried": metricas["combinations_tried"],
                "memory_peak_kb": metricas["memory_peak_kb"],
            }
        )
    ...
    for fila in resultados:
        if fila.get("success") and mas_rapido:
            fila["speedup_vs_slowest"] = round(
                max(tiempos) / fila["execution_time_ms"], 2
            )

    return {
        "size": size,
        "comparison": resultados,
        "fastest_time_ms": mas_rapido,
    }


def calcular_porcentaje_resueltos(
    puzzles: list[dict[str, Any]],
    algoritmo: str = "backtracking",
) -> dict[str, Any]:
    """
    Resuelve un lote de tableros y calcula el porcentaje resuelto correctamente.
    Cada puzzle debe incluir 'size' y 'puzzle'.
    """
    total = len(puzzles)
    resueltos = 0
    ...
    return {
        "algorithm": algoritmo,
        "total_boards": total,
        "solved_correctly": resueltos,
        "success_rate_percent": porcentaje,
        "details": detalle,
    }
...
def _exportar_metricas_excel(metricas: dict[str, Any], destino: Path) -> str:
    """Genera un archivo Excel con hojas de comparación y resumen."""
