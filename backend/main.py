"""
API REST del Sistema de Resolución de Sudoku.

Expone datasets, resolución, generación, grafo de restricciones y métricas.
"""
...
from sudoku_board import (
    celdas_resueltas,
    convertir_a_matriz_9x9,
    generar_tablero_sudoku,
    puzzle_a_tablero,
    tablero_a_api,
    validar_puzzle_inicial,
)
...
from sudoku_metrics import (
    calcular_porcentaje_resueltos,
    comparar_rendimiento_algoritmos,
    exportar_metricas,
    resolver_con_metricas,
)
...
def list_algorithms():
    """Lista los algoritmos de resolución disponibles."""
    return {
        "algorithms": [
            {"key": clave, "name": info["name"]}
            for clave, info in ALGORITMOS.items()
        ]
    }
...
def solve(request: SolveRequest):
    """Requisito 25: resolver Sudoku con métricas completas."""
    size = request.size
    ...
    if algo_key == "brute_force" and size > 9:
        raise HTTPException(
            status_code=400,
            detail="Fuerza Bruta solo está disponible para Sudoku 9×9.",
        )
    ...
    if not validacion["valid"]:
        raise HTTPException(
            status_code=400,
            detail=f"El tablero tiene {validacion['conflicts']} pistas conflictivas.",
        )
    ...
    return {
        "success": True,
        "solution": tablero_a_api(solucion),
        "solved_cells": celdas_resueltas(inicial, solucion),
...
def generate(request: GenerateRequest):
    """Requisito 5 / 25: generar nuevo Sudoku."""
    size = request.size
    if size not in (9, 12, 16):
        raise HTTPException(status_code=400, detail="Tamaño no soportado")
...
def build_graph(request: GraphRequest):
    """
    Requisito 6: modelar grafo de restricciones.
    Requisito 7: opcionalmente exportar nodes.csv y edges.csv para Gephi.
    """
    if request.size != 9:
        raise HTTPException(
            status_code=400,
            detail="El grafo de 81 nodos está definido para Sudoku 9×9.",
        )
    ...
    respuesta: dict[str, Any] = {
        "graph": {
            "node_count": grafo["node_count"],
            "edge_count": grafo["edge_count"],
            "nodes": grafo["nodes"],
            "edges": grafo["edges"],
        }
    }
...
def compare_algorithms(request: CompareRequest):
    """Requisito 15: comparar rendimiento entre algoritmos."""
    if request.size not in (9, 12, 16):
        raise HTTPException(status_code=400, detail="Tamaño no soportado")

    comparacion = comparar_rendimiento_algoritmos(
        request.puzzle, request.size, request.algorithms
    )
    return comparacion
...
def benchmark_datasets(request: BenchmarkRequest):
    """Requisito 20: porcentaje de tableros resueltos correctamente."""
    puzzles = listar_datasets_disponibles(request.size)
    puzzles_completos = []
    for meta in puzzles:
        dataset = obtener_tablero_por_id(meta["id"])
        puzzles_completos.append(
            {"size": dataset["size"], "puzzle": dataset["puzzle"]}
        )
...
def export_metrics_endpoint(request: ExportMetricsRequest):
    """Requisito 21: exportar métricas a JSON, CSV o Excel."""
    if request.format not in ("json", "csv", "xlsx"):
        raise HTTPException(
            status_code=400,
            detail="Formato debe ser 'json', 'csv' o 'xlsx'.",
        )
...
def download_metrics_file(filename: str):
    """Descarga un archivo de métricas previamente exportado."""
...
def download_gephi_file(filename: str):
    """Descarga archivos CSV del grafo para Gephi."""
