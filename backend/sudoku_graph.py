"""
Modelado del Sudoku como grafo de restricciones y exportación para Gephi.

Requisitos cubiertos:
  6. Modelar el tablero como grafo de restricciones (81 nodos y aristas).
  7. Generar nodes.csv y edges.csv para Gephi.
"""
...
def _celdas_misma_unidad(fila: int, columna: int, size: int) -> set[tuple[int, int]]:
    """Devuelve las celdas que comparten fila, columna o subcuadrante."""
...
def modelar_grafo_restricciones(
    board: list[list[str]], size: int = 9
) -> dict[str, Any]:
    """
    Construye un grafo no dirigido donde cada celda es un nodo
    y existe arista entre celdas que deben tener valores distintos.
    Para Sudoku 9×9 el grafo tiene 81 nodos.
    """
    ...
    for fila in range(size):
        for columna in range(size):
            ...
            nodos.append(
                {
                    "id": nodo_id,
                    "label": f"({fila},{columna})={valor}",
                    "row": fila,
                    "col": columna,
                    "value": valor,
                    "fixed": bool(board[fila][columna]),
                }
            )
    ...
    return {
        ...
        "edge_count": len(aristas),
        "nodes": nodos,
        "edges": aristas,
    }


def exportar_archivos_gephi(
    grafo: dict[str, Any],
    directorio_salida: str | Path,
    prefijo: str = "sudoku",
) -> dict[str, str]:
    """
    Escribe nodes.csv y edges.csv en el directorio indicado.
    Devuelve las rutas absolutas de ambos archivos.
    """
    ...
    with ruta_nodos.open("w", newline="", encoding="utf-8") as archivo:
        escritor = csv.DictWriter(
            archivo,
            fieldnames=["Id", "Label", "Row", "Col", "Value", "Fixed"],
        )
    ...
    with ruta_aristas.open("w", newline="", encoding="utf-8") as archivo:
        escritor = csv.DictWriter(
            archivo,
            fieldnames=["Source", "Target", "Type", "Relationship"],
        )
        escritor.writeheader()
            ...
            escritor.writerow(
                {
                    ...
                    "Type": arista["type"],
                    "Relationship": arista["relationship"],
                }
            )

    return {
        "nodes_csv": str(ruta_nodos.resolve()),
        "edges_csv": str(ruta_aristas.resolve()),
    }
