...
def _parsear_matriz_string(matriz: str, size: int) -> list[int]:
    ...
    if len(valores) != total:
        raise ValueError(f'La matriz {matriz[:24]}… no produce {total} celdas (obtenidas: {len(valores)}).')
    return valores
...
def _cargar_desde_excel(ruta: Path) -> dict[str, dict[str, Any]]:
    ...
    for fila in hoja.iter_rows(min_row=2, values_only=True):
        ...
        datasets[dataset_id] = {'id': dataset_id, 'name': f'{size}×{size} — {dificultad} ({dataset_id})', 'size': size, 'difficulty': dificultad, 'puzzle': puzzle, 'source': 'excel', 'sanitized_clues_removed': removidas}
    libro.close()
    return datasets
...
def cargar_datasets(ruta: str | Path | None=None) -> dict[str, dict[str, Any]]:
    ...
    if path.suffix.lower() in ('.xlsx', '.xlsm', '.xls'):
        ...
        raise FileNotFoundError(f'No se encontró {DEFAULT_EXCEL_PATH} ni {FALLBACK_JSON_PATH}.')
    return _DATASETS
...
def listar_datasets_disponibles(size: int | None=None) -> list[dict[str, Any]]:
    ...
    resultado = [{'id': ds['id'], 'name': ds['name'], 'size': ds['size'], 'difficulty': ds.get('difficulty')} for ds in datasets.values()]
    if size is not None:
